...
def sample_frame_indices(count: int, maximum: int) -> list[int]:
    """Stride-sample and keep the newest frame; callers exclude invalid frames."""
    if maximum < 1:
        raise ValueError("The frame sample count must be positive")
    if count <= maximum:
        return list(range(count))
    stride = max(1, count // maximum)
    return [index * stride for index in range(maximum - 1)] + [count - 1]
