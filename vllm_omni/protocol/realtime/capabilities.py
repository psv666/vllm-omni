...
class RealtimeProtocolCapabilities:
    ...
    The defaults are the full set the codec knows how to decode and no turn
    detection, which is the shape of a consumer that accepts every format it is
    handed and only supports ``turn_detection: null``.
    """
    ...
    *,
    ...
    if validate_turn_detection is not None:
        ...
        if turn_detection_error is not None:
            return RealtimeSessionRejection(code="unsupported_turn_detection", message=turn_detection_error)
    return None
